...
var createPrivateApi = module.exports = function(api_key, secret_key, user_agent, nonce_func){
    ...
    var post_query = function(method, params, callback){
        request.post(createPostOption(url() + method, api_key, secret_key, user_agent, nonce_func(), params), function(error, response, body){
            if (!error && response.statusCode == 200) {
                ...
                console.log('error: '+ response.statusCode + ' at ' + method);
    ...
    return {
        ...
        closeTrade : function(currency_pair, action, price, amount, position_id, callback){
            ...
            return post_query('/exchange/orders', params, callback)
        },
        ...
        getLeveragePositions: function(status, callback){
            return get_query('/exchange/leverage/positions', { "status": status }, callback)
        },
